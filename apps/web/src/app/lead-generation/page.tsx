...
function LeadGenerationContent() {
  ...
  // Results State - Initialized from searchParams if specifically redirected from failure
  const [showResults, setShowResults] = useState(
    () => searchParams.get("payment_failed") === "true",
  );
  const [leadCount] = useState(() => {
    const count = searchParams.get("lead_count");
    return count ? parseInt(count) : 1240;
  });
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      ...
      {showToast && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-50 animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="bg-red-600 text-white px-6 py-3 rounded-2xl shadow-2xl flex items-center gap-3 border border-red-500/20 backdrop-blur-sm">
            <svg
              className="w-5 h-5"
              fill="none"
              stroke="currentColor"
